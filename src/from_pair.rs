...
impl<A: ForIRI> FromPair<A> for AnnotatedAxiom<A> {
...
impl<A: ForIRI> FromPair<A> for Axiom<A> {
    const RULE: Rule = Rule::Axiom;
    fn from_pair_unchecked(pair: Pair<Rule>, ctx: &Context<'_, A>) -> Result<Self> {
...
macro_rules! impl_ce_data_cardinality {
    ($ctx:ident, $inner:ident, $dt:ident) => {{
        ...
        let dr = match pair.next() {
            ...
            None => Datatype($ctx.iri(OWL2Datatype::RDFSLiteral.iri_str())).into(),
        };
            ...
            fn from_pair_unchecked(pair: Pair<Rule>, ctx: &Context<'_, A>) -> Result<Self> {
                ...
                for pair in pairs.next().unwrap().into_inner() {
                    ...
                    match inner.as_rule() {
                        ...
                        Rule::Axiom => {
                            ontology.insert(AnnotatedAxiom::from_pair(inner, ctx)?);
                        }
                        rule => {
